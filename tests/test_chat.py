...
from chatterbox import server, client, cli
...
def run_server(temp_users_file):
    ...
    t = threading.Thread(
        target=server.start_server, kwargs={"host": "127.0.0.1", "port": 9020}, daemon=True
    )
    t.start()
    time.sleep(1)
    yield
...
def test_broadcast_message(run_server):
    ...
    msg = drain_until(conn, lambda m: m.get("type") == "broadcast" and m.get("from") == "alice" and "hello world" in m.get("text", ""))
...
def test_private_message_and_list(run_server):
    ...
    drain_until(u1, lambda m: m.get("type") == "broadcast" and "bob" in m.get("text", "") and "joined the chat" in m.get("text", ""))
    drain_until(u2, lambda m: m.get("type") == "broadcast" and "bob" in m.get("text", "") and "joined the chat" in m.get("text", ""))
...
def test_cli_server():
    ...
    with patch('chatterbox.server.start_server') as mock_server:
...
def test_cli_client():
    ...
    with patch('chatterbox.client.main') as mock_client:
...
@patch('socket.socket')
@patch('chatterbox.client.recv_json')
@patch('chatterbox.client.send_json')
